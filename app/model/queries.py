...
def select_table(pd: Page) -> None:
    ...
    query = sql.SQL("SELECT * FROM {tbl};").format(tbl=sql.Identifier(pd.table_name))
...
def update_row(pd: PageData, keys: list, params: tuple) -> None:
    ...
    query = sql.SQL("UPDATE {tbl} " "SET {kwrgs} " "WHERE {pk} = {pv};").format(
...
def truncate_cascade(pd: PageData) -> None:
    ...
    query = sql.SQL("TRUNCATE {tbl} CASCADE;").format(tbl=sql.Identifier(pd.table_name))
...
def use_function(pf: PageFunc, params: tuple):
    ...
    query = sql.SQL(
        "CREATE TEMPORARY TABLE {tbl} AS " "SELECT * FROM {func}({args});"
    ).format(
...
def create_as_select(pd: Page, select_query: str, params: tuple = None):
    ...
    query = sql.SQL(query).format(sql.Identifier(pd.table_name))
...
def insert_updated_table(pd: Page, updated_table: str):
    ...
    query = sql.SQL("CREATE TABLE {cstm_tbl} AS " "SELECT * FROM {updtd_tbl};").format(
        cstm_tbl=sql.Identifier(pd.table_name), updtd_tbl=sql.Identifier(updated_table)
    )
...
def get_func_description(pf: PageFunc, func_name: str):
    ...
    query = sql.SQL(
        "SELECT description " "FROM pg_description " "WHERE objoid = {fnc} :: regproc;"
    ).format(fnc=sql.Placeholder())
    params = (func_name,)
...
def create_ref_table(pf: PageFunc):
    ...
    query = sql.SQL("CREATE TEMPORARY TABLE {tbl}({clmns} varchar);").format(
        tbl=sql.Identifier(pf.table_name),
        clmns=sql.SQL(" varchar, ").join(
            [sql.Identifier(col) for col in pf.ref_columns]
        ),
    )
